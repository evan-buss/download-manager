...
COPY . .
WORKDIR "/src/src/DownloadManager"
RUN dotnet build "DownloadManager.csproj" -c Release -o /app/build

FROM build AS publish
# Install latest LTS
RUN curl -sL https://deb.nodesource.com/setup_lts.x | bash -
RUN apt-get install -y nodejs
RUN dotnet publish "DownloadManager.csproj" -c Release -o /app/publish
